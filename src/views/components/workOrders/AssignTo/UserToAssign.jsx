...
function UserToAssign({
  ...
  return (
    <div className={`assignTo__users__box ${customCLasses}`}>
      <div className="assignTo__users__box__userInfo">
        <img
          ...
          alt="technician"
          className="assignTo__users__box__userInfo--img"
        />

        <div className="assignTo__users__box__userInfo__text">
          <h3>{item?.name}</h3>
          <p className="text-capitalize">{getUserRole(item?.role)}</p>
        </div>
      </div>
      <PrimaryButton
        text={isAssigning ? "Assigning..." : btnText}
        isDisabled={isDisabled}
        customClasses={`solid ${customCLasses}`}
        onClick={() => handleSubmit({ assignedTo: item?._id })}
      />
    </div>
  );
}
