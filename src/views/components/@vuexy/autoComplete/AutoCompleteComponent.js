...
class Autocomplete extends React.Component {
  ...
  onKeyDown = (e) => {
    ...
    const suggestionList = ReactDOM.findDOMNode(this.suggestionList);
  ...
  renderGroupedSuggestion = (arr) => {
    ...
    return arr.map((item, i) => renderSuggestion(item, i));
  };

  // Ungrouped Suggestions
  renderUngroupedSuggestions = () => {
    const {
      filterKey, suggestions, customRender, suggestionLimit,
    } = this.props;
    const {
      onSuggestionItemClick,
      onSuggestionItemHover,
  ...
  renderSuggestions = () => {
    const {
      filterKey, grouped, filterHeaderKey, suggestions,
    } = this.props;
    const {
      renderUngroupedSuggestions,
      state: { userInput },
  ...
  componentDidUpdate(prevProps, prevState) {
    const textInput = ReactDOM.findDOMNode(this.input);
    const { autoFocus, onSuggestionsShown, clearInput } = this.props;
    // For searchbar focus
